...
export default function RoomsNeedIndice({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    setContractData(
      data.filter((s) =>
        checkMonthYear(route.params.monthYear, s.chuki, s.billMonthYear)
      )
    );
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          onSelectRoom ? onSelectRoom(room) && navigation.goBack() : null
        }
      >
  ...
  const checkMonthYear = (date, chuki, billMonthYear) => {
    if (!billMonthYear) {
      return true;
    }
